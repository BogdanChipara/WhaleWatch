"""
download_1candleperhour.py
Downloads only the latest 1-hour candle history window (70 days) for each coin
in COIN_LIST and stores each in SQLite.

This keeps intraday downloads fast and bounded.
...
# Get 1H resolution configuration
RES_CONFIG = RESOLUTIONS["1h"]
...
MAX_HOURLY_HISTORY_DAYS = 70
MAX_HOURLY_HISTORY_SECONDS = MAX_HOURLY_HISTORY_DAYS * 86400

def fetch_candles(symbol: str, start_ts: int, end_ts: int) -> list:
    """Fetch up to 1500 hourly candles for a symbol between start_ts and end_ts."""
...
def download_symbol(symbol: str) -> str | None:
    """Download last 70 days of hourly candles for symbol and store in SQLite."""
    storage_symbol = candle_symbol(symbol, "1h")

    all_candles = {}  # keyed by timestamp to avoid duplicates

    end_ts = int(time.time())
    start_ts = end_ts - MAX_HOURLY_HISTORY_SECONDS

    print(f"[{symbol}] Downloading 1-hour history for last {MAX_HOURLY_HISTORY_DAYS} days...")
...
def main() -> None:
    for symbol in COIN_LIST:
        ...
        if saved:
            # Build intervals immediately after download
            build_result = build_intervals_for_symbol(symbol, resolution="1h")
