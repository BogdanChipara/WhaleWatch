"""
download_1candleper12hours.py
Downloads only the latest 12-hour candle history window (365 days) for each coin
...
RES_CONFIG = RESOLUTIONS["12h"]
CANDLE_TYPE = RES_CONFIG["kucoin_type"]
MAX_12H_HISTORY_DAYS = 365
MAX_12H_HISTORY_SECONDS = MAX_12H_HISTORY_DAYS * 86400

def fetch_candles(symbol: str, start_ts: int, end_ts: int) -> list:
    """Fetch 12-hour candles for a symbol between start_ts and end_ts."""
...
def download_symbol(symbol: str) -> str | None:
    """Download last 365 days of 12-hour candles for symbol and store in SQLite."""
    storage_symbol = candle_symbol(symbol, "12h")

    all_candles = {}

    end_ts = int(time.time())
    start_ts = end_ts - MAX_12H_HISTORY_SECONDS

    print(f"[{symbol}] Downloading 12-hour history for last {MAX_12H_HISTORY_DAYS} days...")
...
def main() -> None:
    for symbol in COIN_LIST:
        ...
        if saved:
            build_result = build_intervals_for_symbol(symbol, resolution="12h")
